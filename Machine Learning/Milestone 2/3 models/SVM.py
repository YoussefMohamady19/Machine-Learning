...
X_train, X_test, y_train, y_test = train_test_split(X_new, Y, test_size=0.2, shuffle=True) # 80% training and 20% test

params_1 = {'C': 0.01,'max_iter': 1000,'penalty': 'l2'}
params_2 = {'C': 1,'max_iter': 5000,'penalty': 'l2'}
params_3 = {'C': 10,'max_iter': 10000,'penalty': 'l2'}


from sklearn.svm import LinearSVC
from sklearn.multiclass import OneVsRestClassifier,OneVsOneClassifier
#svm_linear_ovr = OneVsOneClassifier(LinearSVC(C=1)).fit(X_train, y_train)
start=time.time()
model_1 = OneVsOneClassifier(LinearSVC(**params_1)).fit(X_train, y_train)
model_2 = OneVsOneClassifier(LinearSVC(**params_2)).fit(X_train, y_train)
model_3 = OneVsOneClassifier(LinearSVC(**params_3)).fit(X_train, y_train)
stop=time.time()
...
#save model
model_filename = "SVM.sav"
saved_model = pickle.dump(model_2, open(model_filename,'wb'))
